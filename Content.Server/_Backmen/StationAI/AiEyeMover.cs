...
public sealed class AiEyeMover : Job<object>
{
    ...
    protected override async Task<object?> Process()
    {
        try
        {
            ...
            var gridUid = _transform.GetParentUid(NewPosition.EntityId);

            if (
                //gridUid == null ||                            //эта строка всегда false
                _transform.GetParentUid(_transform.GetMoverCoordinates(core).EntityId) != gridUid ||
            ...
            foreach (var uid in _map.GetAnchoredEntities(gridUid, grid, NewPosition))
            {
