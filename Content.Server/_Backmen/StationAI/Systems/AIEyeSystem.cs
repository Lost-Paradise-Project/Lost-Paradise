using Content.Server._Backmen.StationAI.Systems;
...
public sealed class AIEyePowerSystem : EntitySystem
{
    ...
    {
        if (_uiSystem.TryCloseAll(ent.Owner))
    ...
    private void RemoveMind(EntityUid uid, AIEyeComponent component)    // поскольку нет разницы по какой причине это вызвано, то код вынесен
    {
        if (_uiSystem.TryCloseAll(uid))
        {
            QueueDel(uid);
    ...
    private void OnReturnToCore(Entity<StationAIComponent> ent)
    {
        ...
        _uiSystem.TryCloseAll(ent.Owner);
