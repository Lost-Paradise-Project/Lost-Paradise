...
public sealed partial class LPSalvageSystem
{
    ...
    private void OnConsoleFTLAttempt(ref ConsoleFTLAttemptEvent ev)
    {
        if (!TryComp(ev.Uid, out TransformComponent? xform) ||
            !TryComp<LPSalvageExpeditionComponent>(xform.MapUid, out var salvage))
        {
            return;
        }
    ...
    {
        if (!HasComp<LPSalvageExpeditionComponent>(ev.MapUid) ||
    ...
    {
        if (!TryComp<LPSalvageExpeditionComponent>(args.MapUid, out var component))
            return;
        ...
        if (component.Stage != LPExpeditionStage.Added)
            return;
        ...
        component.Stage = LPExpeditionStage.Running;
    ...
    private void OnFTLStarted(ref FTLStartedEvent ev)
    {
        ...
        if (TryComp<LPSalvageMiningExpeditionComponent>(
        ...
        if (!TryComp<LPSalvageExpeditionComponent>(ev.FromMapUid, out var expedition) ||
            !TryComp<LPSalvageExpeditionDataComponent>(expedition.Station, out var station))
        {
            return;
        }
    ...
    {
        // Generic missions
        var query = EntityQueryEnumerator<LPSalvageExpeditionComponent>();
        ...
        while (query.MoveNext(out var uid, out var comp))
        {
            ...
            if (comp.Stage < LPExpeditionStage.FinalCountdown && remaining < TimeSpan.FromSeconds(45))
            {
                comp.Stage = LPExpeditionStage.FinalCountdown;
                Dirty(uid, comp);
                Announce(uid, Loc.GetString("salvage-expedition-announcement-countdown-seconds", ("duration", TimeSpan.FromSeconds(45).Seconds)));
            }
            else if (comp.Stage < LPExpeditionStage.MusicCountdown && comp.Stream == null && remaining < audioLength) // Frontier
            {
                var audio = _audio.PlayPvs(comp.Sound, uid);
                comp.Stream = audio?.Entity;
                _audio.SetMapAudio(audio);
                comp.Stage = LPExpeditionStage.MusicCountdown;
                Dirty(uid, comp);
                Announce(uid, Loc.GetString("salvage-expedition-announcement-countdown-minutes", ("duration", audioLength.Minutes)));
            }
            else if (comp.Stage < LPExpeditionStage.Countdown && remaining < TimeSpan.FromMinutes(5))
            {
                comp.Stage = LPExpeditionStage.Countdown;
                Dirty(uid, comp);
        ...
        // Structure missions
        var structureQuery = EntityQueryEnumerator<LPSalvageStructureExpeditionComponent, LPSalvageExpeditionComponent>();
        ...
        var eliminationQuery = EntityQueryEnumerator<LPSalvageEliminationExpeditionComponent, LPSalvageExpeditionComponent>();
